...
function correctMassScaleAndExtractSumSpec(
  ...
  referenceSpectrum = getAvgSpectrumFromFile(referenceFile)
  referenceMassScaleMode, referenceMassScaleParameters = getMassCalibParametersFromFile(referenceFile)
  setMassScaleReferenceSpectrum(referenceSpectrum, calibRegions, searchWidth, referenceMassScaleMode, referenceMassScaleParameters, plotControlMass=plotControlMass, testRangeStart=testRangeStart, testRangeEnd=testRangeEnd)
  referenceMassAxis = []
  referenceMassAxis = h5read(referenceFile, "FullSpectra/MassAxis")
  ...
  for j=1:nFiles
    ...
    fileIsBad = false;
    massAxis = []
    massAxis = h5read(totalPath, "FullSpectra/MassAxis")
    time[j] = getTimeFromFile(totalPath)

    avgSpectrum = getAvgSpectrumFromFile(totalPath)
    newParams, success, tbs, ins = recalibrateMassScale(avgSpectrum, referenceSpectrum, calibRegions, searchWidth, referenceMassScaleMode, referenceMassScaleParameters)
    ...
    if (createTotalAvg == true && !fileIsBad)
      ...
      indexesExact = mass2timebin(referenceMassAxis, referenceMassScaleMode, newParams)
      interpolatedSpectrum = interpolate(indexesExact,avgSpectrum)
      ...
      if (storeSubSpectra == true)
        ...
        if (j != nFiles)
          set_dims!(dsetAvgSumSpecs, (dsAvgSumWidth,currDims+1)::Dims)
        end
      end
    end
    if debuglevel > 1   println() end
    for i=(1:nMasses)
      #if debuglevel > 0   println("Processing region mass($(mlow[i]):$(mhigh[i])) --> timebin($(round(mass2timebin(mlow[i],massCalibMode,newParams))):$(round(mass2timebin(mhigh[i],massCalibMode,newParams))))") end
      if (massBorderCalculation == 2)
        centerIndex = Int64(floor(mass2timebin(masslistMasses[i], referenceMassScaleMode, newParams)))
        ...
        subIdxStartExact=mass2timebin(mlow[i],referenceMassScaleMode,newParams)
        subIdxEndExact = mass2timebin(mhigh[i],referenceMassScaleMode,newParams)
    ...
    if (!onlyUseAverages)
      spectrumMultFactor = getSpecMultiplicator(totalPath)
      subSpecStickCps = SharedArray{Float32}(nMasses)
      totalSubSpectra = getSubSpectraCount(totalPath)

      ################## prepare arrays for averaging #########################
      fileInternalLocalAvg = getSubSpectrumFromFile(totalPath,1)
      ...
      if totalSubSpectra >= recalibInterval
        ...
        for avgIdx=2:minimum([recalibInterval totalSubSpectra])
          fileInternalLocalAvg += getSubSpectrumFromFile(totalPath,avgIdx)
        end
        newParams, success, tbs, ins = recalibrateMassScale(fileInternalLocalAvg*spectrumMultFactor, referenceSpectrum, calibRegions, searchWidth, referenceMassScaleMode, referenceMassScaleParameters)
        ######
        fill!(fileInternalLocalAvg,0)
        fileInternalLocalAvgCount = 0
      end
      ...
      for subSpecIdx=1:totalSubSpectra
        subSpectrum = getSubSpectrumFromFile(totalPath,subSpecIdx)
        ...
        if (fileInternalLocalAvgCount > recalibInterval) || fileInternalLocalAvgCount == totalSubSpectra
          ...
          newParams, success, tbs, ins = recalibrateMassScale(fileInternalLocalAvg, referenceSpectrum, calibRegions, searchWidth, referenceMassScaleMode, referenceMassScaleParameters)

          indexesExact = mass2timebin(referenceMassAxis, referenceMassScaleMode, newParams)
          interpolatedSpectrum = interpolate(indexesExact,fileInternalLocalAvg)
        ...
        rawTime = getSubSpectrumTimeFromFile(totalPath,subSpecIdx)
        ...
        if !((j==nFiles) && (subSpecIdx==getSubSpectraCount(totalPath)))
          set_dims!(dsetTimes, (currDimsTime+1,)::Dims)
          set_dims!(dsetStickCps, (currDimsTime+1,dsStickCpsWidth)::Dims)
          set_dims!(dsetStickCpsErr, (currDimsTime+1,dsStickCpsWidth)::Dims)
        end
      end
    end
    println("###################################################################\n\n")
  ...
  h5write(outfilepath, "AvgStickCps", stickcpsFlat')
  h5write(outfilepath, "AvgStickCpsErr", stickcpsErrFlat')
  h5write(outfilepath, "MassList", masslistMasses)
  println("Wrote $(length(masslistMasses)) masses to file.")
  h5write(outfilepath, "AvgStickCpsTimes", timeFlat)
  h5write(outfilepath, "MassAxis", referenceMassAxis)
  if onlyUseAverages
    h5write(outfilepath, "AvgSpectrum", totalAvgSpectrumFlat)
  else
    h5write(outfilepath, "AvgSpectrum", totalAvgSubSpectrumFlat)
  end

  if (createTotalAvg == true)
    #h5write(outfilepath, "SumSpecs", sdata(interpolatedSpectra))
    h5write(outfilepath, "SumSpecMax", sdata(totalMaxSpectrum))
    h5write(outfilepath, "SumSpecMin", sdata(totalMinSpectrum))
  end
  h5write(outfilepath, "ElementNames", masslistElements)
  h5write(outfilepath, "ElementMasses", masslistElementsMasses)
  ...
  h5write(outfilepath, "ElementalCompositions", compFlat)
