...
function getMassCalibParametersFromFile(filename)
  ...
  if HDF5.exists(HDF5.attrs(fh), "InstrumentType")
    ...
    elseif length(setData[1,:])==3
      ...
      println("Error in APiTOFFunctions: MassCalParam is higher than 3. Use only 3 MassCalParams or extend script.")
    end
...
function loadWholeTofData(filename)
      return HDF5.h5read(filename, "/SPECdata/Intensities")
end

function getSubSpectrumFromFile(filename, index; preloadFile = "")
  if spectraCache.filename != filename
      if APiTOFFunctions.preloadFilename == filename
          print("Fetching Preload Future of $(filename)... ")
          spectraCache.content = fetch(APiTOFFunctions.preloadFuture)
          ...
          if prod(size(ds)) < SPEC_CACHE_SIZE_LIMIT
            ...
            #  if HDF5.exists(HDF5.attrs(fh), "InstrumentType")
              spectraCache.content = HDF5.h5read(filename, "/SPECdata/Intensities")
                  #println("size 1 spectraCache.content: $(size(spectraCache.content)) ")
              #else
  ...
  if (preloadFile != "") && (preloadFile != APiTOFFunctions.preloadFilename) && (APiTOFFunctions.preloadFuture == 0) && (nprocs() >=2 )
    fh = HDF5.h5open(preloadFile,"r")
    #if HDF5.exists(HDF5.attrs(fh), "InstrumentType")
    ds = fh["/SPECdata/Intensities"]
        #println("size ds: $(size(ds[:,:])) ")
    #else
    #    ds = fh["/FullSpectra/TofData"]
    #end
    specsize = prod(size(ds))
    HDF5.close(fh)

    if specsize < SPEC_CACHE_SIZE_LIMIT
    ...
    #fh = HDF5.h5open(preloadFilename,"r")
    #if HDF5.exists(HDF5.attrs(fh), "InstrumentType")
  #println("size 2 spectraCache.content: $(size(spectraCache.content)) filename: $preloadFilename")
  subSpectrum = spectraCache.content[:,index]
    #else
...
function getTimeFromFile(filename)
    ...
    if HDF5.exists(HDF5.attrs(fh), "InstrumentType")
      ...
      tUnix = time_LabVIEWTimestamp-((1970-1904)*365+(1970-1904)/4+0.5)*24*3600 # don't forget the leap years
...
for regionindex=1:length(m_calibRegions)
  ...
  if (m_plotControlMass == true)
    ...
    indexesExact = APiTOFFunctions.mass2timebin(crOriginalMasses, m_referenceMassScaleMode, newParams)
